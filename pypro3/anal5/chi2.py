...
# 결과 해석 : 유의확률(p-value) 0.01439 < 유의 수준 0.05 이므로 귀무가설을 기각하고 대립가설을 채택한다.
# 다시 말해서 검정에 사용된 data 값은 우연히 발생된 값이 아니라 어떠한 필연적 원인에 발생된 데이터라고 할 수 있다.
# 이 주사위는 게임에 적합하지 않다.

# 결과 얻기 2 - 카이제곱 분포표 사영
# 검정통계량 X² 값 : 14.20000, df(자유도) : 5(전체 개수 - 1), 임계값 : 11.07(임계값 표로 추출) 
# 카이제곱값이 임계값 바깥쪽에 있기 때문에 귀무가설을 기각하고 대립가설을 채택한다. 
...
# 해석 : p-value = pvalue=0.00039991 < 0.05 귀무가설 기각
# 그러므로 이 결과를 바탕으로 향 후 어떠한 조치를 취하는데 있어서 자료로 쓰임