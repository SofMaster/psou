...
# 대립 가설(H1) : 벼락치기 공부하는 것과 합격 여부는 관계가 있다.
# 귀무 설정 : ... 같다. 다르지 않다. 차이가 없다. 효과가 없다. 보숮적으로 선언
# 대립 설정 : 귀무 가설과 바대로 설정
...
# 검정통계량 카이제곱 = 합(관측값 - 기대값)제곱 / 기대값 : x 축의 임의 지점 값을 구함. 임계치를 기준으로 가설을 판단할 때 사용.
# 기대 도수 = ( 각행의 주변합 ) * ( 각 열의 주변합 ) / 총합

chi2 = (18 - 15) ** 2 / 15 + (7 - 10) ** 2 / 10 + (12 - 15) ** 2 / 15 + (13 - 10) ** 2 / 10
print('카이제곱 값 : ', chi2)  # 3.0
print('자유도 : ', (2 - 1) * (2 - 1))  # (행의 갯수 - 1) * (열의 갯수 - 1)

# 카이제곱표를 통해 임계치 얻기  : df(자유도) = 1 , α = 0.05(신뢰구간 95%)이므로 카이제곱표를 보면 임계치는 = 3.84
# 귀무 채택력 / 귀무 기각력  임계치보다 카이제곱값이 왼쪽에 있으므로 귀무 채택이다.
# 결론 : chi2값 3.0 < 임계치 3.84 이므로 chi2갑은 귀무 채택역 내에 있어 귀무가설을 채택한다.
...
# 결론 : 유의수준(α) 0.05 < p-value:0.557825 이므로 귀무가설 채택 
