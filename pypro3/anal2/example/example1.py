...
aa = DataFrame(np.random.randn(9, 4))
print(aa)
...
aa = DataFrame([[10],[20],[30],[40]],columns = ['numbers'], index = ['a', 'b', 'c', 'd'] )
print(aa)
# b) c row의 값을 가져오시오.
print(aa.loc['c'])
...
bb = DataFrame([[10],[20],[30],[40]],columns = ['numbers'], index = ['a', 'b', 'c', 'd'] )
cc = aa * bb
print(cc)
# f) floats 라는 이름의 칼럼을 추가하시오. 값은 1.5, 2.5, 3.5, 4.5    아래 결과가 나와야 함.
aa.loc[:,'floats'] = [1.5, 2.5, 3.5, 4.5]
print(aa)
# g) names라는 이름의 다음과 같은 칼럼을 위의 결과에 또 추가하시오. Series 클래스 사용.
aa['names'] = Series(['길동', '오정', '팔계', '오공'], index = ['d','a','b','c'])
print(aa)
