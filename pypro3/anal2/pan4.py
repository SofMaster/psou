...
print('DataFrame관련 집계 메소드 : pivot,grouby, pivot_table -------------')
...
print(df.groupby(['city']).sum())  # 위 소스코드를 한 줄로 표현
print(df.groupby(['city']).agg('sum'))
print(df.groupby(['city', 'year']).mean())

# 피벗 테이블은 커다란 표(예 : 데이터베이스, 스프레드시트, 비즈니스 인텔리전스 프로그램 등)의
...
print(df.pivot_table(index = ['city'], aggfunc=np.mean)) # 기본 함수는 평균(mean)

print(df.pivot_table(index = ['city','year'], aggfunc=[len, np.sum])) # 기본 함수는 평균(mean)
