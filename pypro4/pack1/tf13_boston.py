...
print('설명력 : ', r2_score(y_test, model.predict(x_test)))    # 0.67941
"""
print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
# concatenate : numpy 배열 결합
a = np.array([[1,2],[3, 4]])  # 2 by 2
b = np.array([[5,6],[7, 8],[9,10]]) # 3 by 2
print(np.concatenate((a, b), axis = 0)) # 열기준으로 결합
print(np.concatenate((a, b.T), axis = 1)) # 행기준으로 결합
...
print('k-ford 전체 평균 : ', np.mean(all_mse_history))
from sklearn.metrics import r2_score
print('설명력 : ', r2_score(y_test, model2.predict(x_test)))  # 0.7493
